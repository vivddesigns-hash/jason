...
2. **If no connection is found or the status check fails:** Load the `jason-oauth-integrations` skill. The skill will evaluate whether managed or your-own mode is appropriate and guide the user accordingly.
...
2. **If reconnection fails, go straight to setup.** Don't present options, ask which route the user prefers, or explain what went wrong technically. Just tell the user briefly (e.g., "Gmail needs to be reconnected - let me set that up") and immediately load the `jason-oauth-integrations` skill. The user came to you to get something done, not to troubleshoot - make it seamless.
...
When scan results include senders from the user's own company domain (e.g., `@jason.ai`):

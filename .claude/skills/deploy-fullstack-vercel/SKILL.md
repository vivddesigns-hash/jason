...
description: Build and deploy a full-stack app (React frontend + Python/FastAPI backend) or a Jason app to Vercel as a serverless demo with seeded data
...
Deploy a full-stack app with a React/Vite frontend and Python/FastAPI backend to Vercel as a serverless demo, OR deploy a Jason-built app from the library. No auth required - meant for demos, portfolio pieces, and quick showcases.

## When to Use

- User says "deploy this to Vercel", "host this", "publish this"
- User has a project with a frontend + backend they want live
- User wants to deploy a Jason app that uses backend features (data store, custom routes)
- User wants a quick demo deployment (no persistent database needed)

## Authentication

### Jason App Publishing

For publishing Jason apps from the library, use the built-in `publish_page` tool. This is the preferred path — it uses the stored Vercel API token (`vercel/api_token`) via the brokered publish flow without exposing the token to shell commands.
...
For custom projects that need Vercel deployment (not Jason app publishing):
...
## Deploying a Jason App

When the user asks to deploy a Jason app from their library (from `/workspace/data/apps/<app-name>/`):

### 1. Detect Jason Bridge Usage

Check the compiled app for Jason bridge API usage:

```bash
grep -l "window\.jason\.\|jason\.fetch\|jason\.data\|jason\.sendAction" /workspace/data/apps/<app-name>/dist/*.js /workspace/data/apps/<app-name>/dist/*.html 2>/dev/null
```

If found, the app depends on the Jason bridge and needs a shim to work standalone.

### 2. Create Jason Bridge Shim

The app uses `window.jason.*` APIs that are normally injected by the Jason viewer. For standalone deployment, create a `jason-shim.js` file in the app's `dist/` directory that provides browser-native replacements.

**Before writing the shim, read the app's compiled JavaScript** (`dist/main.js` or equivalent) to understand exactly which `window.jason.*` APIs the app calls and what data shapes it expects. The shim must match the app's actual usage — don't guess at signatures.

**Common APIs to shim (implement only what the app actually uses):**

| Bridge API                    | Standalone replacement                                | Notes                                                                                                                    |
| ----------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `jason.data.query()`         | localStorage-backed store                             | Read the app code to determine the record shape — some apps expect `{id, data: {...}}` wrappers, others use flat records |
| `jason.data.create(...)`     | localStorage insert with `crypto.randomUUID()`        | Match the argument signature the app passes (some pass a payload, others pass `{id, ...fields}`)                         |
| `jason.data.update(...)`     | localStorage update                                   | Match the argument signature (typically `(id, payload)`)                                                                 |
| `jason.data.delete(...)`     | localStorage delete                                   | Typically `(id)`                                                                                                         |
| `jason.fetch(path, opts)`    | `console.warn` + return empty success Response        | Custom routes aren't available standalone                                                                                |
| `jason.sendAction(id, data)` | No-op with `console.warn`                             | Surface actions aren't available standalone                                                                              |
| `jason.openLink(url)`        | `window.open(url, '_blank')`                          |                                                                                                                          |
| `jason.widgets.toast(msg)`   | Create a temporary styled `<div>` that auto-dismisses |                                                                                                                          |
| `jason.route`                | `null`                                                | Deep-link routes aren't available standalone                                                                             |

**Structure:** Wrap everything in an IIFE that guards against the real bridge: `(function() { if (window.jason) return; ... })();`

### 3. Inject the Shim into index.html

Add a `<script src="jason-shim.js"></script>` tag in `dist/index.html` BEFORE any `<script type="module">` tags:

```bash
sed -i 's|<script type="module"|<script src="jason-shim.js"></script>\n<script type="module"|' dist/index.html
```
...
```json
{
  "rewrites": [
    {
      "source": "/((?!main\\.js|main\\.css|jason-shim\\.js|assets/).*)",
...
Then deploy using the `publish_page` tool (preferred). For Jason apps, use the built-in app publish flow rather than raw Vercel API calls from shell.
...
| Jason bridge APIs          | Use the jason-shim.js to provide localStorage-backed data + no-op stubs |
